...
# web hero loop: a SEAMLESS crossfade loop, 1080p H.264 (universal browser
...
echo "Building seamless 1080p loop (match=${MATCH}s, xfade=${XFADE}s, out=${D}s)…"
ffmpeg -hide_banner -loglevel error -i "$SRC" -filter_complex "
[0:v]scale=1920:1080:flags=lanczos,format=yuv420p,trim=0:${D},setpts=PTS-STARTPTS,split=3[c1][c2][c3];
...
" -map "[out]" -c:v libx264 -profile:v high -pix_fmt yuv420p -crf 20 -preset slow \
  -movflags +faststart -an "$OUT/portal-loop.mp4" -y

echo "Regenerating matching poster stills (frame 0 of the loop)…"
ffmpeg -hide_banner -loglevel error -i "$OUT/portal-loop.mp4" -frames:v 1 -vf scale=1280:720 /tmp/poster.png -y
ffmpeg -hide_banner -loglevel error -i /tmp/poster.png -c:v libwebp -quality 82 "$OUT/portal-scene.webp" -y
ffmpeg -hide_banner -loglevel error -i /tmp/poster.png -c:v libaom-av1 -still-picture 1 -crf 30 -b:v 0 "$OUT/portal-scene.avif" -y
